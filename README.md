...
## 4. 웹 배포 (PWA — 바탕화면 설치형 웹앱)

```bash
cd flutter_app
flutter build web --release
firebase init hosting        # public 디렉터리: flutter_app/build/web
firebase deploy --only hosting
```

- 배포된 주소를 학생들에게 알려주면, 크롬(안드로이드)은 메뉴 → **"홈 화면에 추가"**,
  아이폰 사파리는 공유 → **"홈 화면에 추가"**로 바탕화면 아이콘처럼 설치됩니다.
  (manifest.json + 서비스 워커가 포함되어 앱처럼 전체 화면으로 실행)
- **자동 로그인**: 한 번 코드+비밀번호로 들어오면 로그아웃 전까지 유지됩니다.
  단, 익명 계정 기반이므로 브라우저 데이터(쿠키/사이트 데이터)를 지우면
  계정이 사라집니다 — 학생들에게 "인터넷 기록 지우기"를 하지 않도록 안내하세요.

## 5. 서버 비용/트래픽 최적화

- **학생 앱은 상시 실시간 리스너를 사용하지 않습니다.** 모든 화면이
  "열 때 1회 읽기 + 행동할 때만 쓰기"로 동작하고, Firestore 오프라인 캐시가
  켜져 있어 같은 데이터는 기기 캐시에서 다시 읽습니다.
- 실시간 스트림은 **교사 대시보드(당일 현황·감정 경고)에만** 유지됩니다 —
  교사가 화면을 열고 있는 동안만 연결됩니다.
- 무거운 집계(미제출 검사, 지역 통계)는 하루 1회 Cloud Functions 스케줄러가
  일괄 처리합니다.
